    ...
    const data = deleteRecipeCategory(recipeCategoryId)
      .then((data) => {
        setCategories((prev: any) =>
          prev.filter((category: any) => category.id !== recipeCategoryId),
        );
    ...
    <div>
      <div className="flex items-center justify-center gap-2 mb-4">
        {categories &&
          categories.map((category: any) => (
            ...
            >
              ...
              {auth.user && (
                <Button
                  onClick={() => deleteRC(category.id)}
                  variant="outline"
                  className="rounded-2xl p-2 h-6 flex items-center justify-center"
                >
                  x
                </Button>
              )}
        ...
        {auth.user && (
          <Select
            value={defaultValue}
            onValueChange={(value) => addRC(recipeId, parseInt(value))}
          >
